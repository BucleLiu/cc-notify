...
function runCodexInit() {
  ...
  // upsertCodexTomlHooks handles: remove stale + append fresh + ensure [features] hooks=true
  const newToml = upsertCodexTomlHooks(toml, CODEX_HOOK_DEFINITIONS);

  let backupPath = null;
  try {
    backupPath = backupFile(CODEX_CONFIG_TOML);
  } catch (e) {
    process.stderr.write(`ccn init --codex: warning — backup failed: ${e.message}\n`);
  }

  try {
    writeCodexConfigToml(newToml);
  } catch (e) {
    process.stderr.write(`ccn init --codex: failed to write config.toml: ${e.message}\n`);
    process.exit(1);
  }

  // Migrate: clean up legacy hooks.json
  const legacyCleaned = cleanupLegacyHooksJson();

  process.stdout.write(`\nConfigured: ${CODEX_CONFIG_TOML}\n`);
  if (backupPath) {
    process.stdout.write(`Backup:      ${backupPath}\n`);
  }
  if (legacyCleaned) {
    process.stdout.write(`Cleaned up:  ${CODEX_HOOKS_JSON} (migrated to config.toml)\n`);
  }
  process.stdout.write('\n');
 ...
 * This is a one-time migration helper; once hooks.json is gone this is a no-op.
 *
 * @returns {boolean} true if anything was cleaned up
 */
function cleanupLegacyHooksJson() {
  ...
  // Remove cc-notify hooks from the JSON structure
  let removedCount = 0;
  for (const [eventKey, eventHooks] of Object.entries(config.hooks)) {
    if (!Array.isArray(eventHooks)) continue;
    for (const group of eventHooks) {
  ...
  if (!hasAnyHooks) {
    try {
      ...
      process.stderr.write(`ccn init --codex: warning — hooks.json cleanup failed: ${e.message}\n`);
    }
    return true;
  }

  // If we didn't remove anything and the file still has hooks from
  // other tools, nothing to do.
  if (removedCount === 0) return false;

  // Partial cleanup — write back with only non-cc-notify hooks remaining
  try {
