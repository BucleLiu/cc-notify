...
`ccn init` 向 `~/.claude/settings.json` 注入以下 5 个 hook 条目（幂等，已存在则跳过）：

| 事件 | matcher | 通知内容 |
|---|---|---|
| `Stop` | — | ✅ Claude Code Finished |
| `Notification` | `permission_prompt` | 🔐 Claude Code Permission（urgent 模式） |
| `PostToolUse` | — | ⏳ Claude Code Working |
| `UserPromptSubmit` | — | ⏳ Claude Code Working |
| `PostCompact` | — | ✅ Claude Code Compressed |

所有 hook 命令引用 `$HOME/.cc-notify/notify.sh`（稳定路径），npm 更新后无需重新运行 `ccn init`。

操作前自动备份：`~/.claude/settings.json.bak.YYYYMMDD-HHMMSS`
